...
async function transferToken2022(amount = 1000) {
  try {
    ...
    transaction.add(
      ...
      // Initialize the token account - use the same instruction as SPL Token
      // but with Token-2022 program ID
      createInitializeAccountInstruction(
        tokenAccountKeypair.publicKey, // account
        mintInfo.mintAddress, // mint
        RECIPIENT_ADDRESS, // owner
        CUSTOM_TOKEN_2022_PROGRAM_ID // Token-2022 program ID
      )
    );
    ...
    // Add mint instruction - use the same instruction as SPL Token
    // but with Token-2022 program ID
    transaction.add(
      createMintToInstruction(
        mintInfo.mintAddress, // mint
...
async function main() {
  ...
  if (args.length > 0) {
    ...
    if (isNaN(providedAmount) || providedAmount <= 0) {
      ...
      console.log('💡 Usage: node transfer-token-2022-working.js [amount]');
      console.log('💡 Example: node transfer-token-2022-working.js 500');
      process.exit(1);
    }
