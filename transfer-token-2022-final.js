...
// Create Token-2022 compatible initialize account instruction
function createToken2022InitializeAccountInstruction(
  account,
  mint,
  owner,
  programId
  ...
  // InitializeAccount instruction discriminator for SPL Token (should work for Token-2022)
...
// Create Token-2022 compatible mint to instruction
function createToken2022MintToInstruction(
  mint,
  destination,
  authority,
  amount,
  ...
  // MintTo instruction: discriminator (1 byte) + amount (8 bytes)
...
async function transferToken2022(amount = 750) {
  try {
    ...
    transaction.add(
      createToken2022MintToInstruction(
        mintInfo.mintAddress, // mint
...
async function main() {
  ...
  if (args.length > 0) {
    ...
    if (isNaN(providedAmount) || providedAmount <= 0) {
      ...
      console.log('💡 Usage: node transfer-token-2022-final.js [amount]');
      console.log('💡 Example: node transfer-token-2022-final.js 500');
      process.exit(1);
    }
...
module.exports = { transferToken2022, connection, CUSTOM_TOKEN_2022_PROGRAM_ID }; 