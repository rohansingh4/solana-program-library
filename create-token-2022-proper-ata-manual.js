...
// Program IDs - Using STANDARD Solana ATA program
const CUSTOM_TOKEN_2022_PROGRAM_ID = new PublicKey('2dwpmEaGB8euNCirbwWdumWUZFH3V91mbPjoFbWT24An');
const STANDARD_ATA_PROGRAM_ID = new PublicKey('ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL');
...
async function createToken2022ManualATA() {
  try {
    const wallet = loadKeypair();
    
    console.log('🚀 Creating Token-2022 with PROPER ATA (Manual Method)...');
    ...
    const tokenMetadata = {
      ...
      symbol: "GPREM",
      description: "A premium token for GorbChain with proper ATA support using Token-2022",
      decimals: 9,
      totalSupply: 1000000,
      ...
      network: "GorbChain",
      version: "2022-proper-ata-manual",
      ataSupported: true,
    ...
    const mintInfo = {
      ...
      createdAt: new Date().toISOString(),
      type: 'token-2022-proper-ata-manual'
    };

    fs.writeFileSync('proper-ata-token-manual.json', JSON.stringify(mintInfo, null, 2));
    console.log('💾 Token info saved to proper-ata-token-manual.json');
...
async function transferWithProperATAManual(mintInfo, recipient, amount) {
  try {
    const wallet = loadKeypair();
    const recipientPubkey = new PublicKey(recipient);
    const mintAddress = new PublicKey(mintInfo.mintAddress);
    
    console.log('\n🚀 Transferring with PROPER ATA (Manual Method)...');
    ...
    console.log(`🌐 Network: GorbChain (Proper ATA Manual)`);
    console.log('=' .repeat(60));
    ...
    const filename = `proper-ata-transfer-${Date.now()}.json`;
...
async function main() {
  try {
    console.log('🎯 Creating Token-2022 with PROPER ATA Support (Manual Method)...');
    
    // Step 1: Create the token
    const mintInfo = await createToken2022ManualATA();
    ...
    const transferResult = await transferWithProperATAManual(mintInfo, recipient, amount);
    
    console.log('\n🎊 COMPLETE SUCCESS WITH PROPER ATA!');
    ...
    console.log(`🎯 This is the CORRECT way to use Token-2022 with ATA!`);
...
module.exports = { 
  createToken2022ManualATA, 
  transferWithProperATAManual,
